...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        // 3D variants — transição e sombra gerenciadas por .btn-3d-*
        default:
          "bg-primary text-primary-foreground btn-3d btn-3d-primary",
        destructive:
          "bg-destructive text-destructive-foreground btn-3d btn-3d-destructive",
        outline:
          "border border-input bg-background hover:bg-accent hover:text-accent-foreground btn-3d btn-3d-outline",
        secondary:
          "bg-secondary text-secondary-foreground hover:bg-secondary/80 btn-3d btn-3d-secondary",
        // Flat variants
        ghost:
